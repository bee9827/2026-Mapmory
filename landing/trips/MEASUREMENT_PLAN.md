...
배포 성공과 운영 GA 수신 확인은 별개다. 2026-09-23 사용자가 전용 스트림 생성을 승인했고,
향상된 측정 OFF를 확인한 `G-P0TDZHRQ6P`로 동의 기반 수집을 활성화하는 변경이다.
현재 운영 배포·실제 수신 확인 여부는 별도 배포 기록을 참조한다.
...
콘솔 맞춤 정의 등록과 운영 수신 확인은 코드 테스트와 별도로 수행하고 배포 기록에 남긴다.
...
2026-09-23 사용자 선택으로 같은 속성 551158914 안에 `Mapmory Trips 실험` 전용 웹 스트림을 만들었다.
스트림 ID `15828403934`, 공개 측정 ID `G-P0TDZHRQ6P`, URL `https://map-mory.com/trips/`.
향상된 측정 OFF, 연결된 사이트 태그 0개를 콘솔에서 확인했다. 기존 `G-MC93CZWLZF`의 설정은 변경하지 않았다.
운영 빌드에는 검증된 전용 ID를 명시하며 공유 파이프라인의 ID를 상속하지 않는다.
`TRIPS_GA_RELEASE_APPROVED=true`여도 참가자 통계 동의 전에는 태그를 로드하지 않는다.
두 웹 스트림을 하나의 연속 여정으로 해석하지 않고 Trips 스트림/이벤트로 한정해 분석한다.
실제 수신 확인 전까지 활성화 코드를 수신 성공으로 표현하지 않는다.
...
5. 배포 전 공유 스트림의 **향상된 측정** 설정을 확인한다. 자동 파일 다운로드/폼/외부 링크
   ...
   설정 조정을 결정한 뒤 활성화한다. 이번에는 전용 스트림의 향상된 측정을 OFF로 생성해 공유 설정 변경을 피했다.
