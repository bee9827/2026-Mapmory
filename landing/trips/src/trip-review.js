...
export function createTripReview({ records, root, photoAlbum, clearAlbums, onBack, onChoose, createNotice, createSurvey,
  ...
  function render() {
    ...
    if (review.mode === 'holding') {
      root.append(el('p', 'result-caveat', '위치와 촬영일을 함께 확인할 수 없어 분류를 보류했어요. 사진 수만으로 여행을 추정하지 않아요.'));
    }
    ...
    details.append(el('summary', '', '어떻게 분류했나요?'),
      el('p', '', '위치·날짜가 확인된 사진과 그 사이의 촬영 시간으로 연결한 사진을 구분해요. 여행 확정이 아닌 자동 제안이며 일상·출장도 포함될 수 있어요. 위치 기준이 없는 사진의 촬영량 기반 추정은 보류 중이에요.'),
