...
The approved production URL is https://map-mory.com/trip/, not a separate subdomain. Keep the main landing at /. Build a separate dist/trip artifact with /trip/ asset URLs while preserving the existing dist/client Sites/root build. Local production verification uses npm run preview:trip at the same port 4174 and /trip/ path; never start a second server. Keep application code separate, and integrate only static trip files into the landing CodeDeploy bundle. Preserve the user's managed CodePipeline, manual approval, backend isolation and release branch protections; do not deploy or retarget PRs without authority.
