...
- Put the primary `여행 묶어서 보기` action immediately after the result heading, before summary statistics, metadata notices and the location/date folders. Keep its green visual connection to location grouping and one explicit home-selection step; a long folder list must never push the action below the fold.
